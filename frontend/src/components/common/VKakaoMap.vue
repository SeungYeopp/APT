  ...
  loadKakaoMapScript()
    .then(() => {
      ...
      if (props.apartments.length > 0) {
        ...
        props.selectApt &&
        props.selectApt.latitude &&
        props.selectApt.longitude
      ) {
        addSingleMarker(props.selectApt);
      } else {
        // 기본 중심 위치로 지도를 설정
        map.setCenter(
          new kakao.maps.LatLng(37.5012986996253, 127.039627288835)
        );
      }
    })
    .catch((error) => {
      console.error(
        "Error loading Kakao map script or initializing map:",
        error
      );
    });
});
...
const addSingleMarker = (selectedApt) => {
  ...
  const position = new kakao.maps.LatLng(
    selectedApt.latitude,
    selectedApt.longitude
  );
...
const loadMarkers = () => {
  ...
  const bounds = positions.value.reduce((bounds, position) => {
    if (
      position.latlng &&
      (position.latlng.getLat() !== 0 || position.latlng.getLng() !== 0)
    ) {
...
<template>
  ...
  <ul id="category">
    <li id="BK9" data-order="0">
      ...
      <img
        src="https://map.pstatic.net/resource/api/v2/image/maps/around-category/bank_category_pc.png?version=21"
        alt=""
      />
      은행
    </li>
    <li id="MT1" data-order="1">
      ...
      <img
        src="https://map.pstatic.net/resource/api/v2/image/maps/around-category/mart_category_pc.png?version=21"
        alt=""
      />
      마트
    </li>
    <li id="PM9" data-order="2">
      ...
      <img
        src="https://map.pstatic.net/resource/api/v2/image/maps/around-category/pharmacy_category_pc.png?version=21"
        alt=""
      />
      약국
    </li>
    <li id="OL7" data-order="3">
      ...
      <img
        src="https://map.pstatic.net/resource/api/v2/image/maps/around-category/oil_category_pc.png?version=21"
        alt=""
      />
      주유소
    </li>
    <li id="CE7" data-order="4">
      ...
      <img
        src="https://map.pstatic.net/resource/api/v2/image/maps/around-category/cafe_category_pc.png?version=21"
        alt=""
      />
      카페
    </li>
    <li id="CS2" data-order="5">
      ...
      <img
        src="https://map.pstatic.net/resource/api/v2/image/maps/around-category/store_category_pc.png?version=21"
        alt=""
      />
      편의점
    </li>
  ...
  <div class="map-type">
    <button
      id="btnUseDistrict"
      class="map-btn"
      @click="toggleMapType('useDistrict')"
    >
      지적편집도
    </button>
    <button id="btnTerrain" class="map-btn" @click="toggleMapType('terrain')">
      지형정보
    </button>
  </div>
</template>
