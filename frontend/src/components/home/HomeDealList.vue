        ...
        categories.map(
            ...
            new Promise((resolve) => {
                ...
                (data, status) => {
                    ...
                    console.error(`카테고리 ${category} 검색 중 오류 발생: ${status}`);
                    resolve([]); // 빈 배열 반환
                    }
                },
                {
...
const averageRating = computed(() => {
    ...
    const total = reviewList.value.reduce((sum, review) => sum + review.rating, 0);
...
const generateStars = (rating) => {
    return Array.from({ length: 5 }, (_, index) => getStarImage(index + 1, rating));
};
...
// 표시할 거래 데이터 계산
const visibleDeals = computed(() => dealList.value.slice(0, visibleCount.value));
...
function processData(dealData) {
    ...
    const sortedKeys = Object.keys(groupedData).sort((a, b) => new Date(b) - new Date(a)); // 최신 거래 순 정렬
    ...
    const avgPrices = recentKeys.map((date) => groupedData[date].totalAmount / groupedData[date].count);
...
<div class="deal-list-view">
    ...
    <button v-if="showBackButton" @click="clickHide" class="back-button">← 뒤로</button>
    <h2>{{ AptNm }}
        <span v-if="reviewList.length" class="average-rating"> ({{ averageRating }} 
            <span class="star" style="color: var(--jonquil);">★</span>)
        </span>
    </h2>
    
    <button v-if="user" @click="openReviewModal" class="review-button">리뷰 작성</button>
    </div>
        
        <div class="chart-container">
            <canvas id="dealChart"></canvas>
        </div>
        <p v-if="dealList.length" class="deal-count">총 거래 내역: {{ dealList.length }}건</p>
            ...
            <table class="deal-table">
                ...
                <tbody>
                    <tr v-for="item in visibleDeals" :key="item.dealId">
                        ...
                        <td>{{ formatDealDate(item.dealYear, item.dealMonth, item.dealDay) }}</td>
                        <td>{{ item.excluUseAr }}</td>
                        <td>{{ item.floor }}층</td>
                    </tr>
                </tbody>
            </table>
            <button v-if="visibleDeals.length < dealList.length" @click="loadMore" class="load-more-button">
                더보기
            </button>
        </div>
        ...
        <div class="facility-section">
            ...
            <div class="category-buttons">
                <button v-for="category in categoryList" :key="category.code" @click="filterByCategory(category.code)"
                    :class="{ active: selectedCategory === category.code }" class="category-button">
                    {{ category.name }}
                </button>
            </div>

            <!-- 시설 목록 -->
            <div v-if="filteredFacilities.length > 0" class="facility-table-container">
                <table class="facility-table">
                    ...
                    <tbody>
                        <tr v-for="facility in filteredFacilities" :key="facility.id">
                            ...
                            <td style="display: flex; justify-content: space-between;">{{ facility.road_address_name || facility.address_name }} 
                                <a :href="facility.place_url" target="_blank" class="button-link">→</a>
                            </td>
                        </tr>
                    </tbody>
                </table>
                ...
                <div v-for="review in reviewList" :key="review.reviewId" class="review-card">
                    <div class="review-header">
                        ...
                        <div class="review-stars">
                            <img v-for="(star, index) in generateStars(review.rating)" :key="index" :src="star"
                                class="star-icon" />
                        </div>
                    </div>
                    <div class="review-content">
                        ...
                        <img
                            ...
                            alt="리뷰 이미지"
                            @click="openModal(`http://localhost:8080${review.imageUrl}`)"
                    ...
                    <div class="review-footer">
                        <div v-if="userId === review.user.id" class="footer-buttons">
                            <button @click="openEditReviewModal(review)" class="edit-button">수정</button>
        ...
        <ReviewModal v-if="showReviewModal" :apt-seq="selectedApt" :apt-name="AptNm" :user-nickname="userNickname"
            :user-id="userId" :editing-review="editingReview" :is-editing="isEditing" @review-saved="handleReviewSaved"
